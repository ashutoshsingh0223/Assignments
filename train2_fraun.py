...
def main(train_batch_size=32, test_batch_size=8, learning_rate=0.0001, epochs=30):
    # Create run_id and a directory
    now = datetime.now()  # current date and time
    run_id = f"augmented-run-{now.strftime('%d-%m-%Y-%H-%M')}"
    ...
    train_set_with_transform = ClassificationDataset(train=True, transform=transforms)
    train_loader_with_transform = DataLoader(dataset=train_set_with_transform, shuffle=True, pin_memory=True,
                                             num_workers=2, batch_size=train_batch_size)
    ...
    for epoch in range(epochs):
        ...
        if epoch % 2 == 0:
            ...
            for data, labels in train_loader_with_transform:
                data = data.to(device)
                labels = labels.to(device)
